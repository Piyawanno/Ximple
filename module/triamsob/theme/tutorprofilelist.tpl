		...
		<div class="main_box_body">
			<div class="main_box_content">
				...
				<div class="tutor_profile">
					<h3 class="title_label"><?=$user['login_name']?></h3>
					<p>
						<label><?=tt('subject')?></label>
						<?php foreach($user['subject'] as $key => $sid):?>
							<a href="<?=SECTION_URI.Q.'triamsob_tutor_subject/'.$sid?>" class="tutor_subject"><?=$subject[$sid]?></a>
						<?php endforeach?>
					</p>
					...
					<?php endif?>
					<p align="right"><strong><a href="<?=SECTION_URI.Q.'triamsob_tutor_profile/'.$uid?>"><?=tt('see more')?></a></strong></p>
				</div>
