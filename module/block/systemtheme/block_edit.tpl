...
<h3 class="list_label"><?=tt('Block Edit')?></h3>
<label><?=tt('Legend')?></label>
<ul>
	<li><img src="<?=ROOT_URI?>files/icon/move.png"/> : <?=tt('Sort and select position by drag-and-drop')?></li>
	<li><img src="<?=ROOT_URI?>files/icon/edit.png"/> : <?=tt('Edit and Configure Block')?></li>
</ul>
<p>
	<?=tt('You can use this form to sort and set blocks position by drag-and-drop. Please, do not forget to click submit.')?>
</p>

<form action="<?=SECTION_URI.Q?>block_save_sort" method="post" onsubmit="getPosition()">
<?php $sortable_id = "";?>
