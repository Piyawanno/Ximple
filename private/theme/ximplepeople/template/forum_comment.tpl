...
<div class="comment">
	<h3 class="small_label">
		<?php if($value['writer_id']['id'] == -1):?>
			<a style="color:white;text-decoration:none;" href="<?=$value['homepage']?>"><?=$value['writer']?></a>
		<?php else:?>
			<a style="color:white;text-decoration:none;" href="<?=SECTION_URI.'profile/'.$value['writer_id']['id']?>"><?=$value['writer']?></a>
		<?php endif?>
	</h3>
	<?php
